...
Noticeably, the idea of _lock slice_ utilized in U-SCL also accommodates with the idea of delegation-style locks by making sure that only one thread is executing the critical section at a given time slice, thus reducing the need for moving shared data around cores to improve performance @scl_ref.  This concept closely mirrors the goal of delegation-style locks, yielding a similar performance enhancement. Nonetheless, it presents a notable drawback: within a valid _lock slice_, no other thread can hold the lock, even when the preceding thread has released it. Consequently, the non-critical section must be brief; otherwise, throughput will be substantially affected. In contrast, delegation-style locks offer comparable performance benefits without the _lock slice_ drawback.
...
On the other hand, most delegation-style locks inherently provide some fairness guarantee. There's no reason that the combiner shall treat their critical sections differently from others', so most of the policy is to enumerate all ready jobs. For example, the combiner of FlatCombining (or the server of RCL) enumerates all the thread lists to check whether a waiter is trying to execute a critical section. This is not the case for a spin-lock. Some of the threads might dominate the lock usage and starve the others. If threads repeatedly reacquired the lock, the thread that is releasing the lock will have some advantage to reacquire the lock as the cache synchronization is faster for them.
...
To mitigate these problems, we modify the original algorithm of FlatCombining and #ccsynch to achieve our usage fairness goal. We majorly adopted two strategies to mitigate the issue.
