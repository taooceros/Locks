...
def main():
    ...
    text('复现分析：分别调用 python3 -m integration.upscaledb.experiments.hypotheses.hypothesis_roles、python3 -m integration.upscaledb.experiments.hypotheses.hypothesis_reservation、python3 -m integration.upscaledb.experiments.hypotheses.hypothesis_bursts 的 --analyze-only --output-root 参数；原分析目录只读，H2 可指定新的 --analysis-root，H3 重分析须使用保留原数据的新目录。报告命令：python3 -m integration.upscaledb.reports.hypothesis_report --input-root .worktree/upscaledb-hypotheses --output-dir 新报告目录。不得通过删除旧结果来重跑。')
