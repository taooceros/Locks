#!/usr/bin/env python3
"""Build pinned UpScaleDB standard variants and separate heterogeneous batch variants.
...
Heterogeneous variants use their own source and binary names and cannot reuse
a standard library without the batch patch.
"""
import argparse
import hashlib
import json
...
BATCH_PATCH = HERE / 'heterogeneous-ops.patch'
HETERO_HARNESS = HERE / 'heterogeneous_clients.cc'
BRIDGE_KINDS = {'bridge_mutex': 0, 'fc': 1, 'fc_pq': 2, 'uscl': 3, 'cfl_local': 4,
                'spinlock': 5, 'mcs': 6, 'ticket': 7, 'clh': 8}
HETERO_BACKENDS = ('native', 'bridge_mutex', 'fc', 'fc_pq', 'uscl', 'cfl_local', 'mcs')
HETERO_VARIANTS = tuple('hetero_' + name for name in
                        (*HETERO_BACKENDS, 'profile',
                         *(name + '_profile' for name in HETERO_BACKENDS if name != 'native')))
VARIANTS = ('native', 'refactored', *BRIDGE_KINDS, 'profile',
            *(name + '_profile' for name in BRIDGE_KINDS), 'test_hooks', *HETERO_VARIANTS)


def source_kind(variant):
    if variant in HETERO_VARIANTS:
...
def source_patches(kind):
    ...
    if kind == 'heterogeneous':
        return (BRIDGE_PATCH, BATCH_PATCH)
...
def verify_source(source, variant, provenance_root=None):
    ...
    if variant == 'heterogeneous':
        result['heterogeneous_patch_sha256'] = digest(BATCH_PATCH)
    return result
...
def verify_library(source, variant, boost_dev, gcc, boost, source_info, require_provenance,
    ...
    if variant in ('refactored', 'refactored_test', 'heterogeneous'):
    ...
    has_batch = any(line.split() and line.split()[-1] == 'ups_dlock_private_native_batch'
                    for line in symbols)
    if has_batch != (variant == 'heterogeneous'):
        raise RuntimeError('Existing library batch ABI does not match ' + variant)
...
def build(variant, jobs, boost, boost_dev, gcc, resume, harness_only, built_sources,
    ...
    built_now = (not harness_only and kind not in built_sources and
                 (not reused or kind not in ('native', 'profile', 'heterogeneous')))
    if built_now:
    ...
    if kind == 'heterogeneous' and (binary.exists() or
                                    (OUT / ('build-' + variant + '.json')).exists()):
        raise RuntimeError('Existing heterogeneous binary/manifest; choose a fresh --output-root: ' + str(binary))
    ...
    harness = HETERO_HARNESS if kind == 'heterogeneous' else HARNESS
    args = [str(gcc / 'bin/g++'), '-std=c++17', '-O3', '-g', '-DNDEBUG', '-pthread',
            '-I' + str(source / 'include'), '-I' + str(HERE)]
    backend = variant.removeprefix('hetero_') if kind == 'heterogeneous' else variant
    if backend == 'profile':
        args.append('-DUPS_NATIVE_PROFILE=1')
    bridge_kind = 2 if variant == 'test_hooks' else BRIDGE_KINDS.get(
        backend.removesuffix('_profile'))
    profile = backend.endswith('_profile')
    ...
    baseline = None
    if kind == 'heterogeneous' and bridge_kind is None:
        baseline = {
            'name': 'patched_native_env_mutex',
            'primary_single_operations': 'original public UpScaleDB find/insert API on Env mutex',
            'profile_single_operations': 'same original protected body on Env mutex with pre-unlock timing',
            'batch': 'one Env-mutex acquisition for eight original internal insert bodies',
            'source_patch_sha256': digest(BRIDGE_PATCH),
            'batch_patch_sha256': digest(BATCH_PATCH),
        }
    elif bridge_kind == 3:
        baseline = {
    ...
    if kind == 'heterogeneous':
        manifest['hashes']['heterogeneous_patch_sha256'] = digest(BATCH_PATCH)
...
def main():
    ...
    if args.variants:
        ...
        selected = (VARIANTS[:-(len(HETERO_VARIANTS) + 1)] if args.variant == 'all'
                    else (args.variant,))
    global OUT
    ...
    if reuse_root is not None and any(source_kind(name) == 'heterogeneous' for name in selected):
        parser.error('heterogeneous variants require their own batch-patched DB; no --reuse-build-root')
